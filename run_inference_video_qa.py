...
def extract_complete_sentences(output):
    """Extract complete sentences ending with punctuation."""
    sentences = re.split(r'(?<=[.!?])', output)  
    complete_sentences = [s.strip() for s in sentences if s.strip() and s[-1] in ".!?"]
...
def llava_inference(
    ...
    # Get prompt
    prompt = get_prompt(model, conv_mode, question, cot_caption, caption)
            ...
            if os.path.exists(video_path):
                ...
                if args.enable_cot:
                    ...
                    for i in range(len(CoT_questions)):
                        ...
                        output = extract_complete_sentences(output)
                ...
                output = output.replace("image", "video").replace("Image", "Video")
...
def parse_args():
    ...
    parser.add_argument("--answer_max_new_tokens", type=int, default=128, help="Max new tokens for final answer generation")
    # D-CoDe frame selection parameters
    parser.add_argument("--select_frames", type=int, default=25, help="Number of frames to select (N)")
    parser.add_argument("--uniform_ratio", type=float, default=0.8, help="Ratio for uniform sampling (e.g., 0.8)")
