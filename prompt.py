...
def get_multiple_choice_prompt(model, conv_mode, question, candidates, cot_caption=False, captions=None): 
    if conv_mode == "multiple_choice_allvideo_v4":
        if cot_caption:
            prompt = "You are a helpful expert in video analysis. Answer the question. USER: <image>\nThe input consists of a sequence of key frames from a video.\nQuestion: %s\n ASSISTANT:\nA short sentence: "
            prompt = prompt % (question)
        else:
            # Handle None captions
            caption_text = captions if captions else ""
            prompt = "You are a helpful expert in video analysis. Select the best option to answer the question. USER: <image>\nThe input consists of a sequence of key frames from a video. " + caption_text + " \nQuestion: %s\nOptions:\n%sOnly give the best option. \nASSISTANT:\nAnswer: Best option:("
